const { print, on, off, move } = require("./lib/utils.js");
...
class ElectronFindRenderer {
  ...
  initialize() {
    ...
    if (!ipcRenderer.sendSync("init-find")) {
      print("[FindInPage] Failed to initialize.");
      return false;
    }

    this[findBox] = createElement("find-box");
    this[findInput] = createElement("find-input", "input");
    this[findMatches] = createElement("find-matches");
    this[findCase] = createElement("find-case");
    this[findBack] = createElement("find-back");
    this[findForward] = createElement("find-forward");
    this[findClose] = createElement("find-close");
...
function getUserConfig(options) {
  this[config].offsetTop =
    typeof options.offsetTop === "number" ? `${options.offsetTop}px` : "5px";
  this[config].offsetRight =
    typeof options.offsetRight === "number"
      ? `${options.offsetRight}px`
      : "5px";
  this[config].boxBgColor =
    typeof options.boxBgColor === "string" ? options.boxBgColor : "#fff";
  this[config].boxShadowColor =
    typeof options.boxShadowColor === "string"
      ? options.boxShadowColor
      : "#909399";
  this[config].inputColor =
    typeof options.inputColor === "string" ? options.inputColor : "#606266";
  this[config].inputBgColor =
    typeof options.inputBgColor === "string" ? options.inputBgColor : "#f0f0f0";
  this[config].inputFocusColor =
    typeof options.inputFocusColor === "string"
      ? options.inputFocusColor
      : "#c5ade0";
  this[config].textColor =
    typeof options.textColor === "string" ? options.textColor : "#606266";
  this[config].textHoverBgColor =
    typeof options.textHoverBgColor === "string"
      ? options.textHoverBgColor
      : "#eaeaea";
  this[config].caseSelectedColor =
    typeof options.caseSelectedColor === "string"
      ? options.caseSelectedColor
      : "#c5ade0";
}
function setBoxStyle() {
  this[
    findBox
  ].style.cssText = `position:fixed; top:-110%; z-index: 3001; max-height:48px; min-height:30px; 
...
function setInputStyle() {
  this[
    findInput
  ].style.cssText = `width:168px; outline:0; border:1px solid ${this[config].inputBgColor}; 
...
function setMatchesStyle() {
  ...
  this[
    findMatches
  ].style.cssText = `color:${this[config].textColor}; font-size:14px; display:flex; align-items:center; 
...
function setCaseStyle() {
  ...
  this[
    findCase
  ].style.cssText = `font-size:14px; font-weight:700; cursor:pointer; -webkit-user-select:none; color:${this[config].textColor}; 
...
function setBackStyle() {
  this[
    findBack
  ].style.cssText = `cursor:pointer; -webkit-user-select:none; position: relative; height: 20px; width: 20px; border-radius:2px;
    overflow: hidden; display: inline-block; background:${this[config].boxBgColor}; border:0px solid ${this[config].boxBgColor};`;

  let backLine = createElement("find-back-line");
  ...
  let backCover = createElement("find-back-cover");
...
function setForwardStyle() {
  ...
  let forwardLine = createElement("find-forward-line");
  ...
  let forwardCover = createElement("find-forward-cover");
...
function setCloseStyle() {
  this[
    findClose
  ].style.cssText = `cursor:pointer; -webkit-user-select:none; position: relative; height: 20px; width: 20px;
    overflow: hidden; display: inline-block; background:${this[config].boxBgColor}; border-radius:2px;`;

  let closeInner1 = createElement("find-close-inner1");
  ...
  let closeInner2 = createElement("find-close-inner2");
...
function createEventHandler() {
  ...
  this[backMouseenter] = function () {
    ...
    this[findBack].style[
      "border"
    ] = `0px solid ${this[config].textHoverBgColor}`;
  }.bind(this);
  this[events].push({
    ele: this[findBack],
    name: "mouseenter",
    fn: this[backMouseenter],
  });
  ...
  this[forwardMouseenter] = function () {
    ...
    this[findForward].style[
      "border"
    ] = `0px solid ${this[config].textHoverBgColor}`;
  }.bind(this);
  this[events].push({
    ele: this[findForward],
    name: "mouseenter",
    fn: this[forwardMouseenter],
  });
...
function onInput() {
  setTimeout(() => {
    ...
    if (text && text !== this[lastText]) {
      ...
      ipcRenderer.send("start-find", [text, true, this[matchCase]]);
...
function findKeep(forward) {
  if (!ipcRenderer.sendSync("is-finding")) return;
...
function onBackClick() {
  this[action] = "back";
  wrapInput(this[findInput], this[findCase], 100);
  ipcRenderer.send("find-next", [false, this[matchCase]]);
}

function onForwardClick() {
  this[action] = "forward";
  wrapInput(this[findInput], this[findCase], 100);
  ipcRenderer.send("find-next", [true, this[matchCase]]);
}

function onCloseClick() {
  this.closeFindWindow() ? ipcRenderer.send("stop-find") : "";
}

function onResult() {
  ipcRenderer.on("result", (_, [activeMatch, matches]) => {
...
module.exports = ElectronFindRenderer;
